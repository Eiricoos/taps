...
#[derive(Debug)]
pub enum SelectionPropertyType {
...
impl SelectionProperties {
    ...
    pub fn set(&mut self, property: &str, value: SelectionPropertyType) {
        match property.replace("_", "").to_lowercase().as_str() {
            "reliability" => {
                if let SelectionPropertyType::Preference(preference) = value {
            ...
            "interface" => {
                if let SelectionPropertyType::InterfaceSet(preference_set) = value {
            ...
            "pvd" => {
                if let SelectionPropertyType::PvdSet(preference_set) = value {
            ...
            "multipath" => {
                if let SelectionPropertyType::MultipathPreference(multipath_preference) = value {
            ...
            "advertisesaltaddr" => {
                if let SelectionPropertyType::Bool(preference) = value {
            ...
            "direction" => {
                if let SelectionPropertyType::Direction(direction) = value {
    ...
    pub fn require(&mut self, property: &str) {
        self.set(
            property,
            SelectionPropertyType::Preference(Preference::Require),
        );
    }
    ...
    pub fn prefer(&mut self, property: &str) {
        self.set(
            property,
            SelectionPropertyType::Preference(Preference::Prefer),
        );
    }
    ...
    pub fn no_preference(&mut self, property: &str) {
        self.set(
            property,
            SelectionPropertyType::Preference(Preference::NoPreference),
        );
    }
    ...
    pub fn avoid(&mut self, property: &str) {
        self.set(
            property,
            SelectionPropertyType::Preference(Preference::Avoid),
        );
    }
    ...
    pub fn prohibit(&mut self, property: &str) {
        self.set(
            property,
            SelectionPropertyType::Preference(Preference::Prohibit),
        );
    }
}
...
mod tests {
    ...
    fn initiate_and_set_selection_properties() {
        ...
        selection_properties.set("reliability", SelectionPropertyType::Preference(Avoid));
        ...
        selection_properties.set(
            "interface",
            SelectionPropertyType::InterfaceSet(interface_preference),
        );
        println!(
        ...
        selection_properties.set(
            "multipath",
            SelectionPropertyType::MultipathPreference(MultipathPreference::Active),
        );
        assert_eq!(MultipathPreference::Active, selection_properties.multipath);

        // Testing setting a bool
        selection_properties.set("advertisesaltaddr", SelectionPropertyType::Bool(true));
        assert_eq!(true, selection_properties.advertises_altaddr);

        // Testing setting a direction
        selection_properties.set(
            "direction",
            SelectionPropertyType::Direction(Direction::UnidirectionalSend),
        );
        assert_eq!(
